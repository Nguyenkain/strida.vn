...
abstract class Gravity_Forms_Multilingual{
    ...
    function gform_pre_render( $form, $ajax ) {
		...
		foreach ( $form['fields'] as $id => &$field ) {
            ...
            $field = $this->maybe_translate_placeholder( $field, $st_context, $form_st_id, $form );
		}
    ...
    private function maybe_translate_placeholder( $field, $st_context, $form_st_id, $form ) {
        if ( isset( $field->placeholder ) && $field->placeholder ) {
            $field->placeholder = icl_t(
                $st_context,
                "{$form_st_id}_{$field->type}-{$field->id}-placeholder",
                $field->placeholder
            );
        }
        ...
        if ( isset( $field->inputs ) && $field->inputs ) {
            foreach ( $field->inputs as $key => $input ) {
                if ( isset( $input[ 'placeholder' ] ) && $input[ 'placeholder' ] ) {
                    $field->inputs[ $key ][ 'placeholder' ] = icl_t(
                        $st_context,
                        "{$form_st_id}"
                        . $this->_sanitize_string_name(
                            "_{$field->type}-{$field->id}-placeholder-input-{$input['id']}",
                            $form
                        ),
