...
class GFML_TM_API extends Gravity_Forms_Multilingual {
	...
	protected function register_placeholders( $form_package, $form_field, $form ) {
		if ( isset( $form_field->placeholder ) ) {
			...
			if ( isset( $form_field->inputs ) ) {
				foreach ( $form_field->inputs as $key => $input ) {
					if ( isset( $input[ 'placeholder' ] ) && $input[ 'placeholder' ] ) {
						$input_str_name_parts    = $string_name_parts;
						$input_str_name_parts[ ] = 'input';
						$input_str_name_parts[ ] = $input[ 'id' ];
						$string_input_name       = implode( '-', $input_str_name_parts );
						$this->register_gf_string(
							$input[ 'placeholder' ],
							$string_input_name,
							$form_package,
